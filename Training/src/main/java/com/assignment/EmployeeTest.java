...
public class EmployeeTest {
	...
	public static void main(String[] args) {
		...
		employees.stream().filter(x -> x.getMobileNumbers().contains(new MobileNumber("3333")))
		...
		employees.stream().map(x -> x.toString().toLowerCase()).collect(Collectors.toList())
				.forEach(System.out::println);
		 ...
		 * 
		 * https://howtodoinjava.com/sort/sort-arraylist-objects-comparable-
		 * comparator/
		 */
		...
		Integer[] array123 = list123.stream().map(v -> Integer.valueOf(v)).toArray(Integer[]::new);
	}
}
